                  ...
                  <li key={item.id} className="flex items-start justify-between gap-3 py-2.5">
                    <div className="flex min-w-0 items-start gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        {item.occurredAt && <p className="text-[11px] text-muted-foreground">{relativeTimeShort(item.occurredAt)}</p>}
                      </div>
                    </div>
                    <Link
                      to={link.to}
                    ...
                    >
                      {item.canAct ? item.actionLabel : "View"} <ArrowRight className="h-3 w-3" />
                    </Link>
                  </li>
                );
              })}
            </ul>
            {items.length > limit && <p className="pt-2 text-xs text-muted-foreground">+{items.length - limit} more</p>}
            {partialFailure && <p className="pt-2 text-[11px] text-muted-foreground">Some sources couldn&apos;t be checked; this list may be incomplete.</p>}
          </>
        )}
      </CardContent>
    </Card>
