...
export function RevenueAnalyticsSection({ kpis }: { kpis: AnalyticsKpis }) {
  return (
    <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="rounded-md border p-3">
            <p className="text-lg font-semibold">{formatMoneyByCurrency(kpis.revenue_total, "$0.00")}</p>
            <p className="text-xs text-muted-foreground">Total recorded revenue</p>
          </div>
          <div className="rounded-md border p-3">
            <p className="text-lg font-semibold">{formatMoneyByCurrency(kpis.revenue_attributed, "$0.00")}</p>
            <p className="text-xs text-muted-foreground">Attributed to a known campaign</p>
          </div>
          <div className="rounded-md border p-3">
            <p className="text-lg font-semibold">{formatMoneyByCurrency(kpis.revenue_unattributed, "$0.00")}</p>
            <p className="text-xs text-muted-foreground">Unattributed (organic/manual/unknown)</p>
          </div>
        </div>
        <p className="mt-3 text-xs text-muted-foreground">
          Recorded from revenue_events (real cash events), not opportunity estimated/actual value - the two are tracked separately and are never assumed equal.
        </p>
      </CardContent>
    </Card>
  );
