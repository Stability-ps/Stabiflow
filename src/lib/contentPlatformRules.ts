...
// the Media Library and post composer (badge: "meets requirements" / "too
// small" etc.) without a network round trip per asset.
//
// The AUTHORITATIVE copy that actually blocks scheduling/publishing lives in
// supabase/functions/_shared/contentPlatformRules.ts and is enforced
// server-side by content-schedule-post, content-series-activate, and
// content-publish-worker. This file must be kept in sync with it by hand -
// it can never itself gate what actually gets published, only what the UI
// shows before that server-side check runs.
...
export type ValidationResult = { platform: ContentPlatformKey; valid: boolean; aspectRatio: number; failures: ValidationFailure[] };
...
export const ALL_PLATFORM_KEYS: ContentPlatformKey[] = ["facebook_feed", "instagram_feed", "linkedin_company_page"];
