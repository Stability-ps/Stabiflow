  ...
  return useQuery({
    ...
    queryFn: async (): Promise<WorkspaceIntegrationRow[]> => {
      const { data, error } = await supabase
        ...
        .select("id, provider, status, connected_at, disconnected_at, last_health_check_at, last_health_check_status, last_health_check_message, last_success_at, token_expires_at")
